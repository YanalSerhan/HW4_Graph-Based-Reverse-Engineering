...
def main():
    ...
    md.extend([
        ...
        "|--------|--------|-------|-------|",
        f"| Total nodes | {len(graph_before.nodes)} | {len(graph_after.nodes)} | {fmt(delta_nodes)} |",
        f"| Total edges | {len(graph_before.edges)} | {len(graph_after.edges)} | {fmt(delta_edges)} |",
        f"| Error nodes | {len(diff.error_nodes_before)} | {len(diff.error_nodes_after)} | {fmt(delta_error)} ✅ |",
        f"| Ambiguous edges | {len(diff.ambiguous_edges_before)} | {len(diff.ambiguous_edges_after)} | {fmt(delta_amb)} ✅ |",
    ])

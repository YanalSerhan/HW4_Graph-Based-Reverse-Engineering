...
class ApiGatekeeper:
    ...
    def _cleanup_timestamps(self, current_time: float) -> None:
        ...
        while (
            self.request_timestamps_minute and current_time - self.request_timestamps_minute[0] > 60
        ):
            self.request_timestamps_minute.popleft()
        while (
            self.request_timestamps_hour and current_time - self.request_timestamps_hour[0] > 3600
        ):
    ...
    def _wait_for_rate_limit(self) -> None:
        ...
        while True:
            ...
            if (
                len(self.request_timestamps_minute) < self.config.rpm
                and len(self.request_timestamps_hour) < self.config.rph
            ):
                break
            time.sleep(1)
    ...
    def execute(self, api_call: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            while retries <= self.config.max_retries:
                ...
                try:
                    # api_call returns an object with token/cost attributes or a standard dict
                    ...
                    logger.warning(
                        "API Call Failed",
                        extra={"timestamp": current_time, "error": str(e), "success": False},
                    )
    ...
    def get_queue_status(self) -> QueueStatus:
        ...
        return QueueStatus(
            queue_depth=self.queue_depth, backpressure=self.queue_depth >= self.max_queue_depth
        )

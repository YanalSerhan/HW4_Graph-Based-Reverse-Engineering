...
class TestCodeInspectorAgent:
    def test_run_returns_results_list(self, simple_graph, mock_repo_path, token_counter, mock_llm_call):
    ...
    def test_ambiguous_insight_escalated(self, simple_graph, mock_repo_path, token_counter, mock_llm_call):
        insight = ArchitecturalInsight(
            title="Ambiguous",
            observation="obs",
            relation="rel",
    ...
    def test_validate_edge_ambiguous_returns_escalated(self, simple_graph, mock_repo_path, token_counter, mock_llm_call):
    ...
    def test_validate_edge_extracted_skips_ast(self, simple_graph, mock_repo_path, token_counter, mock_llm_call):
        ...
        outcome = agent.validate_edge(edge, simple_graph)
        assert outcome in {ValidationOutcome.SKIPPED, ValidationOutcome.CONFIRMED, ValidationOutcome.DISPUTED}

    def test_ast_call_found_returns_confirmed(self, tmp_path: Path, token_counter, mock_llm_call, simple_graph):
        ...
        agent = CodeInspectorAgent(repo_path=repo, token_counter=token_counter, llm_call=mock_llm_call)
        outcome = agent.validate_edge(edge, simple_graph)
        assert outcome == ValidationOutcome.CONFIRMED

    def test_empty_insights_returns_empty_results(self, simple_graph, mock_repo_path, token_counter, mock_llm_call):
        agent = _make_agent(mock_repo_path, token_counter, mock_llm_call)

...
def test_reduction_calculation(tmp_path):
    ...
    with (
        patch("graph_rev_eng.services.token_experiment.call_openai", side_effect=mock_call_openai),
        ...
        patch(
            "graph_rev_eng.services.token_experiment.Path.read_text", return_value="dummy content"
        ),
    ):
        te.main()
...
def test_call_openai_httperror():
    ...
    mock_error = urllib.error.HTTPError(
        url="", code=400, msg="Bad Request", hdrs=None, fp=MagicMock()
    )

...
def mock_environment():
    ...
    with patch("graph_rev_eng.services.confusion_matrix.ConfigManager") as mock_config, \
...
def test_confusion_matrix_counts(mock_environment):
    ...
    with patch("graph_rev_eng.services.confusion_matrix.FACTS", test_facts), \
...
def test_confusion_matrix_precision_recall_f1(mock_environment):
    """Test precision/recall/F1 computation with a known set (e.g. 3 TP, 1 FP, 0 FN -> precision 0.75, recall 1.0)."""
    test_facts = [
        ("Fact TP1", True),  # TP
        ("Fact TP2", True),  # TP
        ("Fact TP3", True),  # TP

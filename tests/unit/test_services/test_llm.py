...
from graph_rev_eng.services.llm import OpenAILLM, LLMResponse
...
def test_llm_successful_response():
    ...
    mock_response.read.return_value = b'{"choices": [{"message": {"content": "mocked response"}}], "usage": {"prompt_tokens": 10, "completion_tokens": 20}}'
...
def test_llm_routes_through_gatekeeper():
    ...
    mock_response = MagicMock()
    mock_response.read.return_value = b'{"choices": [{"message": {"content": "gatekeeper test"}}], "usage": {"prompt_tokens": 1, "completion_tokens": 1}}'
    mock_response.__enter__.return_value = mock_response

...
"""
from graph_rev_eng.services.graph_models import Graph, GraphNode, GraphEdge
...
def test_extra_node_removed():
    """Test a graph with an extra node produces one node in nodes_removed when going from more->fewer."""
    g_before = Graph()
    g_before.nodes["n1"] = GraphNode(node_id="n1", label="Node 1", node_type="module", file_path="f1")
    g_before.nodes["n2"] = GraphNode(node_id="n2", label="Node 2", node_type="module", file_path="f2")
    
    g_after = Graph()
    g_after.nodes["n1"] = GraphNode(node_id="n1", label="Node 1", node_type="module", file_path="f1")
...
def test_error_node_removed():
    ...
    g_before = Graph()
    g_before.nodes["e1"] = GraphNode(node_id="e1", label="SyntaxError", node_type="error", file_path="f1")
    g_before.nodes["n1"] = GraphNode(node_id="n1", label="Node 1", node_type="module", file_path="f1")
...
def test_architectural_health_score_computation():
    ...
    g_after = Graph()
    g_after.nodes["n1"] = GraphNode(node_id="n1", label="Node 1", node_type="module", file_path="f1")
    g_after.nodes["n2"] = GraphNode(node_id="n2", label="Node 2", node_type="module", file_path="f2")
...
from graph_rev_eng.services.graph_differ import main, build_graph_for_state, save_graph
import json
...
def test_graph_differ_main():
    with patch("graph_rev_eng.services.graph_differ.build_graph_for_state") as mock_build, \
